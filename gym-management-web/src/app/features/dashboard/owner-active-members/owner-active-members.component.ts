...
export class OwnerActiveMembersComponent implements OnInit {
  isLoading = true;
  ...
  fetchMembers(appendForMobile = false): void {
    ...
    this.memberService.getActiveMembersList(this.filters).subscribe({
      next: (response) => {
      ...
      error: () => {
        this.errorMessage = 'Unable to load active users.';
        this.isLoading = false;
        this.isLoadingMoreMobile = false;
      }
    });
